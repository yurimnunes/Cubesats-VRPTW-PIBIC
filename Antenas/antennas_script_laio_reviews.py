...
weeks = [1]
year = 2018
num_tracks_per_week = 10
dsn_data = generate_dsn_data(weeks, year, num_tracks_per_week)

with open("toy_problem.json", "w") as file:
...
class Graph:
    ...
    def max_total_time(self):
        """Get maximum total time across all tracks acording to all antennas resource windowns"""
        max_time = 0
        for track_id in self.instance.tracks:
            track_data = self.instance.track_nodes[track_id]
            for antenna, windows in track_data['resource_windows'].items():
                for window in windows:
                    max_time = max(max_time, window[1])
        return max_time
...
instance = Instance()
instance.load_data("toy_problem.json")
...
class ColumnGenerationScheduler:
    ...
    def _setup_bucket_graphs(self):
        ...
        for antenna in self.antennas:
            ...
            print(f"Full nodes for antenna {antenna}: {[node.identifier for node in full_nodes]}")
            ...
            bg = baldes.BucketGraph(full_nodes, int(ub), 1)
    ...
    def _reconstruct_routes(self):
        """Para cada antena, percorre a rota escolhida 
           e determina start/end reais de cada job."""
        self.routes = {}  # { antenna: [ {job, start, end}, ... ] }

        for antenna in self.antennas:
            # encontre o idx de coluna com x = 1 (ou > tol)
            sel = [(idx, self.colunas[antenna][idx])
                   for (a, idx), var in self.x.items()
                   if a == antenna and var.X > 1e-6]
            if not sel:
                continue

            # Se houver múltiplas colunas escolhidas (x contínuo),
            # você pode iterar em todas; aqui assumimos x ∈ {0,1}
            for idx, column in sel:
                route = []
                prev_node = f"vs_{antenna}"
                current_time = 0

                for job in column:
                    # 1) tempo de viagem desde prev_node
                    if job == 'start':
                        continue
                    if job == 'end':
                        job = f"ve_{antenna}"
                    travel = self.graph._get_travel_time(prev_node, job)
                    travel = 0

                    # 2) encontre a janela em que cabe:
                    windows = self.graph.nodes[job]['resource_windows'][antenna]
                    # escolhe a primeira janela que permita começar após current_time+travel
                    for (win_start, win_end) in windows:
                        earliest_start = max(win_start, current_time + travel)
                        if earliest_start + self.graph.nodes[job]['duration'] <= win_end:
                            start = earliest_start
                            end   = start + self.graph.nodes[job]['duration']
                            break
                    else:
                        raise RuntimeError(f"Não foi possível encaixar {job} em {antenna}")
                ...
                # guarde a rota desta coluna
                self.routes.setdefault(antenna, []).append(route)
    ...
    def _create_master_problem(self):
        ...
        for antenna in self.antennas:
            for idx, col in enumerate(self.colunas[antenna]):
                ...
                self.x[(antenna, idx)] = master.addVar(
        ...
        for antenna in self.antennas:
            master.addConstr(
                gp.quicksum(self.x[(antenna, idx)] 
    ...
    def run_column_generation(self, max_iterations=50):
        ...
        for iter in range(max_iterations):
            ...
            self.solution = {a: {} for a in self.antennas}
            for (antenna, idx), var in self.x.items():
                # var.X é o valor ótimo (pode ser zero)
                self.solution[antenna][idx] = var.X

            # se quiser imprimir na hora:
            # for antenna, sol_dict in self.solution.items():
            #     print(f"Antenna {antenna}:")
            #     for idx, val in sol_dict.items():
            #         print(f"  coluna {idx}: x = {val:.6f}  →  jobs = {self.colunas[antenna][idx]}")
            ...
            added = 0
            for antenna in self.antennas:
                print(f"Pricing for antenna {antenna}")
                #dual_values = [
                #    duals.get(f"job_{node.identifier}", 0)
                #    for node in self.nos[antenna][1:-1]
                #]
                dual_values = []
                for i in range(len(self.nos[antenna])):
                    name = self.nos[antenna][i].identifier
                    print(name)
                    if name == "start" or name == "end":
                        continue
                    dual_values.append(duals.get(f"job_{self.nos[antenna][i].identifier}", 0))
                print(dual_values)
                # if iter == 0: 
                ...
                labels = []
                labels = self.buckets[antenna].phaseFour()
                print(labels)
                for label in labels:
            ...
            if added == 0:
                ...
                for antenna, sol_dict in self.solution.items():
                    ...
                    for idx, val in sol_dict.items():
                        print(f"  coluna {idx}: x = {val:.6f}  →  jobs = {self.colunas[antenna][idx]}")
                print("Objective value:", self.master_obj)
                break
...
print("Current OS location:", os.getcwd())
instance = Instance()
instance.load_data("toy_problem.json")
graph = Graph(instance)
scheduler = ColumnGenerationScheduler(graph)
scheduler.print_instance()
routes = scheduler.run_column_generation()
print("Final solution:")
for antenna, antenna_routes in routes.items():
    print(f"\n=== Antena {antenna} ===")
    for route in antenna_routes:
        print("Rota:")
        for leg in route:
            print(f"  Job {leg['job']}: {leg['start']} → {leg['end']}")